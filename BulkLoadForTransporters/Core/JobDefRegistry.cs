...
namespace BulkLoadForTransporters.Core
{
    ...
    {
        // 新增一个委托，定义了注册方法的“形状”
        public delegate void DefRegistration(Dictionary<ThingDef, JobDef> map);
        // 新增一个公共事件，让外部模块可以“订阅”
        public static event DefRegistration OnRegisterDefs;
        ...
        /// </summary>
        public static  HashSet<JobDef> LoadingJobs;
        ...
        static JobDefRegistry()
        {
            ...
            // HACK: 依赖原版的ThingRequestGroup来动态识别所有类型的运输仓和传送门。
            ...
            {
                //if (def is VehicleDef || typeof(VehiclePawn).IsAssignableFrom(def.thingClass))
                //{
                //    JobDefMap[def] = LoadVehicle;
                //    continue;
                //}

                if ((blueprintGroup.Includes(def) || frameGroup.Includes(def)) && !Compatibility_Utility.IsReplaceStuffFrameDef(def))
                {
            ...
            OnRegisterDefs?.Invoke(JobDefMap);
        }

...
namespace BulkLoadForTransporters.Core.Adapters
{
    ...
    public class ConstructionGroupAdapter : IManagedLoadable
    {
        ...
        public static ConstructionGroupAdapter TryCreate(IConstructible primaryTarget, Pawn pawn)
        {
            var primaryTargetThing = primaryTarget as Thing;
            if (primaryTargetThing == null || !primaryTargetThing.Spawned || primaryTargetThing.Map == null)
            {
                // 如果“种子”工地本身就是无效的（例如，null, 未生成, 或不在地图上 )
                return null;
            }

            if (primaryTarget.IsCompleted())
            {
                return null;
            }

            // 只有通过了预检，才真正创建实例。
            return new ConstructionGroupAdapter(primaryTarget, pawn);
        }

        private ConstructionGroupAdapter(IConstructible primaryTarget, Pawn pawn)
        {
            ...
            this._jobTargets = sitesInGrid
                ...
                !t.IsForbidden(pawn) &&
                !Compatibility_Utility.IsIncompatibleConstructionThing(t) &&
